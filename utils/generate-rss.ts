...
export default async function generateRss(): Promise<void> {
  const site_url = `${process.env.SITE_URL || "https://avinashmurugappan.dev"}`;
  ...
  const author = {
    ...
    link: "https://avinashmurugappan.dev",
  };

  const feedOptions = {
    title: "RSS Feed - Avinash Murugappan",
    description: "Avinash Murugappan's personal blog",
    id: site_url,
    link: site_url,
